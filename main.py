...
def main(args):
    ...
    snapshot_folder = os.path.abspath(args.snapshot_folder)  if load_snapshot else None
   ...
   # Initialize variables for heuristic bootstrapping and exploration probability
    ...
    def process_actions():
        ...
        while True:
            if nonlocal_variables['executing_action']:
                ...
                best_push_conf = max(np.max(push_predictions_Q1),np.max(push_predictions_Q2))


                best_grasp_conf = max(np.max(grasp_predictions_Q1),np.max(grasp_predictions_Q2))
                ...
                if nonlocal_variables['primitive_action'] == 'push':
                    ...
                    grasp_Q1_max = np.max(grasp_predictions_Q1)
                    grasp_Q2_max = np.max(grasp_predictions_Q2)
                    best_grasp_conf = max(grasp_Q1_max,grasp_Q2_max)
                ...
                trainer.q_functions_log.append([int(critic_type)])
                logger.write_to_log('critic_type', trainer.q_functions_log)
    ...
    while True:
        ...
        if not exit_called:
            ...
            push_predictions_Q1, push_predictions_Q2 , push_angles_full,push_angles, log_pis_next_push, grasp_predictions_Q1,grasp_predictions_Q2, grasp_angles_full, grasp_angles,log_pis_next_grasp = trainer.forward(color_heightmap, valid_depth_heightmap, is_volatile=True)
        ...
        if 'prev_color_img' in locals():
            ...
            # Backpropagate
            trainer.backprop(prev_color_heightmap, prev_valid_depth_heightmap, prev_primitive_action, prev_best_pix_ind, label_value,prev_push_angles_,prev_grasp_angles,prev_critic_type)

            # Adjust exploration probability
            if not is_testing:
                explore_prob = max(0.5 * np.power(0.9998, trainer.iteration) + 0.05 ,0.15) if explore_rate_decay else 0.5
            ...
            if experience_replay and not is_testing:
                ...
                if sample_ind.size > 0:
                    ...
                    with torch.no_grad():
                        sample_push_predictions_Q1, sample_push_predictions_Q2 , push_angles_full,push_angles, log_pis_next_push, sample_grasp_predictions_Q1,sample_grasp_predictions_Q2, grasp_angles_full, grasp_angles,log_pis_next_grasp  = trainer.forward(sample_color_heightmap, sample_depth_heightmap, is_volatile=True)
                    ...
                    if sample_primitive_action == 'push':
                        trainer.predicted_value_log[sample_iteration] = [max(np.max(sample_push_predictions_Q1),np.max(sample_push_predictions_Q2))]
                        # trainer.label_value_log[sample_iteration] = [new_sample_label_value]
                    elif sample_primitive_action == 'grasp':
                        trainer.predicted_value_log[sample_iteration] = [max(np.max(sample_grasp_predictions_Q1),np.max(sample_grasp_predictions_Q2))]
                        # trainer.label_value_log[sample_iteration] = [new_sample_label_value]
            ...
            if not is_testing:
                ...
                if trainer.iteration % 250 == 0:
                    logger.save_model(trainer.iteration, trainer.model, method)
                    logger.save_model(trainer.iteration, trainer.push_actor, "push_actor")
                    logger.save_model(trainer.iteration, trainer.push_critic1_target, "push_critic1_target")
                    logger.save_model(trainer.iteration, trainer.push_critic2_target, "push_critic2_target")

                    logger.save_model(trainer.iteration, trainer.grasp_actor, "grasp_actor")
                    logger.save_model(trainer.iteration, trainer.grasp_critic1_target, "grasp_critic1_target")
                    logger.save_model(trainer.iteration, trainer.grasp_critic2_target, "grasp_critic2_target")


                    if trainer.use_cuda:
                        trainer.model = trainer.model.cuda()
                        trainer.push_actor = trainer.push_actor.cuda()
                        trainer.push_critic1_target = trainer.push_critic1_target.cuda()
                        trainer.push_critic2_target = trainer.push_critic2_target.cuda()

                        trainer.grasp_actor = trainer.grasp_actor.cuda()
                        trainer.grasp_critic1_target = trainer.grasp_critic1_target.cuda()
                        trainer.grasp_critic2_target = trainer.grasp_critic2_target.cuda()
        ...
        #prev_network_to_update =nonlocal_variables['Q_Network_to_update']
        #prev_push_predictions = push_predictions.copy()
        #prev_grasp_predictions = grasp_predictions.copy()
        prev_best_pix_ind = nonlocal_variables['best_pix_ind']
        prev_critic_type = nonlocal_variables['critic_type']


                
        prev_push_angles_ = prev_push_angles.copy()
        prev_grasp_angles_ = prev_grasp_angles.copy()
...
if __name__ == '__main__':
    ...
    parser.add_argument('--snapshot_folder', dest='snapshot_folder', action='store')
