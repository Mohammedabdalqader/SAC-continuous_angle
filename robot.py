...
class Robot(object):
    ...
    def open_gripper(self, gripper_open_with,asynch=False):
        ...
        if self.is_sim:
            ...
            vrep.simxSetJointForce(self.sim_client, RG2_gripper_handle, gripper_motor_force, vrep.simx_opmode_blocking)
            vrep.simxSetJointTargetVelocity(self.sim_client, RG2_gripper_handle, gripper_motor_velocity, vrep.simx_opmode_blocking)
            while gripper_joint_position < (0.02 + gripper_open_with) : # Block until gripper is fully open
                sim_ret, gripper_joint_position = vrep.simxGetJointPosition(self.sim_client, RG2_gripper_handle, vrep.simx_opmode_blocking)
    ...
    def grasp(self, position, heightmap_rotation_angle,gripper_open_with, workspace_limits):
        print('Executing: grasp at (%f, %f, %f)' % (position[0], position[1], position[2]))

        if self.is_sim:

            # Compute tool orientation from heightmap rotation angle
            tool_rotation_angle_yaw = (heightmap_rotation_angle[0] % np.pi) - np.pi/2  
            tool_rotation_angle_pitch = (heightmap_rotation_angle[1] % np.pi) + np.pi/4
            ...
            rotation_step = 0.3 if (tool_rotation_angle_yaw - gripper_orientation[1] > 0) else -0.3
            num_rotation_steps = int(np.floor((tool_rotation_angle_yaw - gripper_orientation[1])/rotation_step))
            ...
            for step_iter in range(max(num_move_steps, num_rotation_steps)):
                ...
                vrep.simxSetObjectOrientation(self.sim_client, self.UR5_target_handle, -1, (np.pi/2,gripper_orientation[1] + rotation_step*min(step_iter,num_rotation_steps), tool_rotation_angle_pitch), vrep.simx_opmode_blocking)
            vrep.simxSetObjectPosition(self.sim_client,self.UR5_target_handle,-1,(tool_position[0],tool_position[1],tool_position[2]),vrep.simx_opmode_blocking)
            vrep.simxSetObjectOrientation(self.sim_client, self.UR5_target_handle, -1, (np.pi/2, tool_rotation_angle_yaw,tool_rotation_angle_pitch), vrep.simx_opmode_blocking)

            # Ensure gripper is open
            self.open_gripper(gripper_open_with)
    ...
    def push(self, position, heightmap_rotation_angle,push_distance, workspace_limits):
        ...
        if self.is_sim:
            ...
            push_direction = np.asarray([push_orientation[0]*np.cos(heightmap_rotation_angle[0]) - push_orientation[1]*np.sin(heightmap_rotation_angle[0]), push_orientation[0]*np.sin(heightmap_rotation_angle[0]) + push_orientation[1]*np.cos(heightmap_rotation_angle[0])])
